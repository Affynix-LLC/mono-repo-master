...
if (!CF_TOKEN || !CF_ZONE) {
  console.warn('[Cloudflare] Missing environment variables - DNS creation disabled (test mode)');
}
 ...
 */
async function dnsRecordExists(fqdn) {
  try {
 ...
 */
async function createDnsRecord(fqdn) {
  try {
    await axios.post(
      CF_API,
      {
 ...
 *  }
 */
export async function ensureSubdomainForCategory(subdomain) {
  const fqdn = `${subdomain}.affynix.com`;

  if (!CF_TOKEN || !CF_ZONE) {
    console.log(`[Cloudflare] Test mode - would create ${fqdn}`);
