...
# Script to update all subdomain pages with fixes from template
# Fixes: dropdown in header, modal animations, hydration issues

echo "🔧 Updating all subdomain pages with template fixes..."
...
for page in "${PAGES[@]}"; do
    if [ -f "$page" ]; then
        ...
        # Add mounted state after function declaration
        sed -i '' 's/export default function.*Page() {/export default function & {\n  const [mounted, setMounted] = useState(false);/' "$page"
  ...
  useEffect(() => {\
    setMounted(true);\
        ...
        # Remove any existing domain switcher (fixed position)
        sed -i '' '/Domain Switcher/,/^      )}/d' "$page"
        
        # Add dropdown to header (after search input)
...
echo "✅ All subdomain pages updated!"
