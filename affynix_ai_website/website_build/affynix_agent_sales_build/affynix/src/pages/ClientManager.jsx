...
export default function ClientManager() {
  ...
  const createClientMutation = useMutation({
    mutationFn: async (clientData) => {
      ...
      try {
        await base44.functions.invoke('hubspot', {
          action: 'sync_contact',
          client: client
        });
      } catch (error) {
  ...
  const updateClientMutation = useMutation({
    mutationFn: async ({ id, data }) => {
      const updatedClient = await base44.entities.Client.update(id, data);
  ...
  const handleSyncToHubSpot = async (client) => {
    ...
    try {
      const result = await base44.functions.invoke('hubspot', {
        action: 'sync_contact',
        client: client
      });
      ...
      if (result.data.success) {
        ...
        alert(`✗ Failed to sync: ${result.data.error}`);
      }
    } catch (error) {
  ...
  const handleBatchSync = async () => {
    ...
    try {
      ...
      if (result.data.success) {
        ...
        alert(`✗ Batch sync failed: ${result.data.error}`);
      }
    } catch (error) {
      alert(`✗ Error: ${error.message}`);
    } finally {
