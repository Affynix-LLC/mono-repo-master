...
export default function Settings() {
  ...
  const { data: configs = [] } = useQuery({
    queryKey: ['configs'],
    queryFn: () => api.entities.AppConfiguration.list(),
    enabled: !!user,
  });

  const updateConfigMutation = useMutation({
    mutationFn: ({ id, data }) => api.entities.AppConfiguration.update(id, data),
    onSuccess: () => {
  ...
  const createConfigMutation = useMutation({
    mutationFn: (data) => api.entities.AppConfiguration.create(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['configs'] });
    },
  });
