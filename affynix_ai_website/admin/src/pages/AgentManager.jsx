...
export default function AgentManager() {
  ...
  const { data: agents = [] } = useQuery({
    queryKey: ['agents'],
    queryFn: () => api.entities.Agent.list('-created_date'),
    enabled: !!user,
  });
  ...
  const createAgentMutation = useMutation({
    mutationFn: (agentData) => api.entities.Agent.create(agentData),
    onSuccess: () => {
  ...
  const updateAgentMutation = useMutation({
    mutationFn: ({ id, data }) => api.entities.Agent.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['agents'] });
      setDialogOpen(false);
      setEditingAgent(null);
  ...
  const deleteAgentMutation = useMutation({
    mutationFn: (id) => api.entities.Agent.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['agents'] });
    },
  });
